...
namespace AMath
{
    ...
    {
        private ByteArray bits;
        private bool Negative;

        public string Bits => bits.ToString();
        public int Sign => ByteArray.Eq(bits, ByteArray.Empty) ? 0 : Negative ? -1 : 1;

        public static readonly BigInteger Zero = 0,
                                          One = 1,
                                          Ten = 10;

        public BigInteger(ulong value) : this(ByteArray.Empty, false)
        {
        ...
        public BigInteger(ByteArray bits, bool negative)
        {
            this.bits = bits;
        ...
        private static ByteArray AddPositive(ByteArray left, ByteArray right)
        {
            ByteArray carry, summand = left, addend = right;
        ...
        private static ByteArray SubPositive(ByteArray left, ByteArray right)
        {
            if (right >= left) return ByteArray.Empty;
            ByteArray borrow, minuend = left, subtrahend = right;
        ...
        {
            ByteArray result = ByteArray.Empty,
        ...
        public static BigInteger DivRem(BigInteger left, BigInteger right, out BigInteger remainder)
        {
            ...
            ByteArray dividend = left.bits, divisor = right.bits,
                quotient = ByteArray.Empty;

            int d = dividend.Length - divisor.Length;
            divisor <<= dividend.Length - divisor.Length;

            while (d-- >= 0/*ByteArray.Gte(dividend, right.bits)*/)
            {
                quotient <<= 1;
                if (ByteArray.Gte(dividend, divisor))
                {
                    dividend = SubPositive(dividend, divisor);
                    quotient |= ByteArray.Single;
                }
                divisor >>= 1;
            }
        ...
        public static bool Gt(BigInteger left, BigInteger right) =>
            left.Negative != right.Negative? right.Negative : ByteArray.Gt(left.bits, right.bits);

        /// <summary> Lower then </summary>
        /// <returns> <paramref name="left"/> < <paramref name="right"/> </returns>
        public static bool Lt(BigInteger left, BigInteger right) =>
            left.Negative != right.Negative ? left.Negative : ByteArray.Lt(left.bits, right.bits);
        ...
        public static bool Eq(BigInteger left, BigInteger right) =>
            left.Negative == right.Negative && ByteArray.Eq(left.bits, right.bits);
        ...
        public static BigInteger operator -(BigInteger value) => new(value.bits, !value.Negative);
        public static BigInteger operator ~(BigInteger value) => new(ByteArray.Not(value.bits), !value.Negative);
        ...
        public static BigInteger operator &(BigInteger left, BigInteger right) => new(ByteArray.And(left.bits, right.bits), left.Negative && right.Negative);
        public static BigInteger operator ^(BigInteger left, BigInteger right) => new(ByteArray.Xor(left.bits, right.bits), left.Negative != right.Negative);
        public static BigInteger operator |(BigInteger left, BigInteger right) => new(ByteArray.Or(left.bits, right.bits), left.Negative || right.Negative);

        public static BigInteger operator <<(BigInteger value, int offset) => new(ByteArray.Shift(value.bits, -offset), value.Negative);
        public static BigInteger operator >>(BigInteger value, int offset) => new(ByteArray.Shift(value.bits, offset), value.Negative);
...
diff --git a/AMath/Discontinued/BigInteger.cs b/AMath/Discontinued/BigInteger.cs
